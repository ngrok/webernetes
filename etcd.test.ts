...
import { Etcd, prefixRangeEnd } from "./etcd";
...
function isFakeEtcd(client: Etcd | Etcd3): client is Etcd {
  return client instanceof Etcd;
}
...
describe.each([
  { name: "fake etcd", factory: () => new Etcd(new Clock()) },
  { name: "real etcd", factory: createRealEtcd },
])("$name", ({ factory }) => {
...
describe.each([
  { name: "fake etcd", factory: () => new Etcd(new Clock(), { retainedRevisions: 1 }) },
  { name: "real etcd", factory: createRealEtcd },
])("$name compaction", ({ factory }) => {
  let client: Etcd | Etcd3;
  let startingRevision: number;
  ...
  it("rejects reads at a compacted revision even on an empty range", async () => {
    await expect(client.getAll().prefix("nonexistent/").revision(revision(1)).exec()).rejects.toThrow(
      /compacted/i,
    );
  });
  ...
  it("rejects watches on an empty range at a compacted revision", async () => {
    const watcher = await client
      .watch()
      .prefix("nonexistent/")
      .startRevision(String(revision(1)))
      .create();
    await expect(nextEvent<Error>(watcher, "error")).resolves.toMatchObject({
      message: expect.stringMatching(/^Watcher canceled:/),
    });
...
describe("Etcd (fake)", () => {
  ...
  it("compact_revision is set on the canceled data event when watch fails due to compaction", async () => {
    ...
    const canceled = await new Promise<{ canceled: boolean; compact_revision: string }>(
      (resolve, reject) => {
        const timer = setTimeout(
          () => reject(new Error("timed out waiting for canceled data event")),
          2000,
        );
        watcher.on("data", (res) => {
          if (res.canceled) {
