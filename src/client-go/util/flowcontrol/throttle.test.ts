...
import { background, withTimeout } from "../../../go/context";
import {
...
it("multithreaded throttling", async () => {
	const r = newTokenBucketRateLimiter(100, 1);
	let taken = 0;
	let finished = false;
	let start: () => void = () => {};
...
it("wait", async () => {
	const r = newTokenBucketRateLimiter(0.0001, 1);

	const [ctx, cancelFn] = withTimeout(background(), 1000);
	const err = await r.wait(ctx);
	cancelFn();
	expect(err).toBeUndefined();

	const [ctx2, cancelFn2] = withTimeout(background(), 1000);
	const err2 = await r.wait(ctx2);
	cancelFn2();
	expect(err2).toBeDefined();
});

// Models staging/src/k8s.io/client-go/util/flowcontrol/throttle_test.go fakeClock.
function newFakeClock(): Clock {
	const clock = new Clock();
	clock.pause();
	return clock;
}
